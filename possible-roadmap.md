...
> Oversized agent backlog for `jtgsystems/computer-repair`. This file intentionally contains 500 candidate roadmap items so bots always have something to pick up.
...
## Basis
- Repository description: Computer repair toolkit - Diagnostic & repair utilities
- Detected stack: Node.js
- Profile guess: `automation-tool`
- Inferred stage: `productization`
- Key paths: `src/`, `app/`, `lib/`, `components/`, `scripts/`, `public/`
- Workflow files present: `dependabot-automerge.yml`
- Read-first signals: `CLAUDE.md`, `package.json`, `next.config.js`, `tailwind.config.ts`, `components.json`, `.github/workflows/`
- Detected commands: install=`npm install`, lint=`npm run lint`, build=`npm run build`

## Section Map
- **Inputs, Config, And Validation**: `src/`, `scripts/`, `package.json`
- **Core Engine And Runtime Flow**: `src/`, `app/`
- **Outputs, Reports, And Artifacts**: `public/`
...
Scope: `src/`, `scripts/`, `package.json`
